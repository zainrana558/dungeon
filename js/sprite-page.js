...
(function() {
  ...
  const motionProfiles = {
    hero: { cadence: 0.048, amplitude: 2.4, smear: 3, trail: 'rgba(142, 234, 255, 0.22)' },
    monster: { cadence: 0.036, amplitude: 3.1, smear: 4, trail: 'rgba(255, 77, 199, 0.20)' }
  };
  ...
  function liquidOffset(time, index, profile) {
    const wave = Math.sin(time * profile.cadence + index * 0.71);
    const crossWave = Math.sin(time * profile.cadence * 0.47 + index * 1.37);
    return {
      x: (wave * profile.amplitude) + (crossWave * 0.7),
      y: Math.cos(time * profile.cadence * 0.82 + index * 0.53) * profile.amplitude
    };
  }

  function drawPreviewStage(ctx, time, profile) {
    px(ctx, 0, 0, PREVIEW_W, PREVIEW_H, '#0b0b14');
    for (let i = 0; i < 18; i += 1) {
      const wave = Math.sin((time * 0.018) + i * 0.8) * 3;
      px(ctx, i * 7 - 4, 76 + wave, 8, 2, i % 2 ? '#25253a' : '#31314a');
    }
    for (let i = 0; i < 9; i += 1) {
      const drift = liquidOffset(time, i, profile);
      px(ctx, 10 + i * 11 + drift.x, 15 + drift.y * 0.45, 2, 2, profile.trail);
    }
  }

  function renderAnimatedFrame(ctx, entry, drawFn, poses, profile, time) {
    const loop = (time * profile.cadence * 0.28) % poses.length;
    const poseIndex = Math.floor(loop);
    const nextPoseIndex = (poseIndex + 1) % poses.length;
    const blend = easeInOutSine(loop - poseIndex);
    drawPreviewStage(ctx, time, profile);

    for (let smear = profile.smear; smear > 0; smear -= 1) {
      const offset = liquidOffset(time - smear * 7, smear, profile);
      ctx.globalAlpha = 0.09 * smear;
      drawFn(ctx, entry, poseIndex, 28 + offset.x - smear, 12 + offset.y);
    }

    const swim = liquidOffset(time, poseIndex, profile);
    ctx.globalAlpha = 0.42;
    drawFn(ctx, entry, nextPoseIndex, 28 + swim.x * blend, 12 + swim.y * blend);
    ctx.globalAlpha = 1;
    drawFn(ctx, entry, poseIndex, 28 + swim.x, 12 + swim.y);

    ctx.fillStyle = '#d8d4ff';
    ctx.font = '6px monospace';
    ctx.fillText(poses[poseIndex].toUpperCase(), 6, 90);
  }

  function createPreview(entry, drawFn, poses, profile) {
    ...
    canvas.animateSprite = (time) => renderAnimatedFrame(ctx, entry, drawFn, poses, profile, time);
    return canvas;
  }
  ...
  function appendSection(titleText, entries, drawFn, poses) {
    ...
    entries.forEach((entry) => {
      ...
      previewLabel.className = 'sprite-preview-label';
      previewLabel.textContent = 'Liquid-motion preview: sine-eased smear frames, drifting afterimages, and readable combat silhouettes.';
      card.append(title, meta, createPreview(entry, drawFn, poses, titleText === 'Heroes' ? motionProfiles.hero : motionProfiles.monster), previewLabel, renderSheet(entry, drawFn, poses));
  ...
  const animatedCanvases = Array.from(document.querySelectorAll('.sprite-preview-canvas'));
  const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  function animate(time) {
    animatedCanvases.forEach((canvas) => canvas.animateSprite(time));
    if (!reduceMotion) {
